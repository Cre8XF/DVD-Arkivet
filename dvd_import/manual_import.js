...
window.loadManual = async function () {
  ...
  for (const movie of movies) {
    ...
    div.innerHTML = `
      ...
      ${(movie.poster || movie.cover) ? `<img src="${movie.poster || movie.cover}" alt="Poster">` : "<p>❌ Ingen plakat</p>"}
      <p><strong>Sjanger:</strong> ${movie.genres || (Array.isArray(movie.genre) ? movie.genre.join(", ") : movie.genre) || "-"}</p>
      <p><strong>Format:</strong> ${movie.format || "-"}</p>
      <p><strong>Regissør:</strong> ${movie.director || "-"}</p>
      <p><strong>Lengde:</strong> ${movie.runtime || "-"}</p>
      <p><strong>IMDb:</strong> ${movie.imdbRating || movie.imdb || "-"}</p>
